<?php include_once './src/Components/Header2.php';  ?>


<!-- Here goes all the content -->
<h2 class="subtitle">See the best man clothes</h2>

<div class="card__container">
	<div class="card">
		<div class="card__image">
			<img src="/src/assets/saco.jpg" alt="poncho">
		</div>
		<div class="card__info">
			<h3>Poncho super cool</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione error repellat explicabo deleniti modi fugit, veniam laboriosam, beatae minus inventore similique qui esse. Voluptatibus magnam fuga doloribus, praesentium natus possimus.</p>
			<div class="buy__container">
				<span class="price">$39.99</span>
				<button class="btn btn__bag">Add to the bag</button>
			</div>
		</div>
	</div>
	<div class="card">
		<div class="card__image">
			<img src="/src/assets/saco.jpg" alt="poncho">
		</div>
		<div class="card__info">
			<h3>Poncho super cool</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione error repellat explicabo deleniti modi fugit, veniam laboriosam, beatae minus inventore similique qui esse. Voluptatibus magnam fuga doloribus, praesentium natus possimus.</p>
			<div class="buy__container">
				<span class="price">$39.99</span>
				<button class="btn btn__bag">Add to the bag</button>
			</div>
		</div>
	</div>
	<div class="card">
		<div class="card__image">
			<img src="/src/assets/saco.jpg" alt="poncho">
		</div>
		<div class="card__info">
			<h3>Poncho super cool</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione error repellat explicabo deleniti modi fugit, veniam laboriosam, beatae minus inventore similique qui esse. Voluptatibus magnam fuga doloribus, praesentium natus possimus.</p>
			<div class="buy__container">
				<span class="price">$39.99</span>
				<button class="btn btn__bag">Add to the bag</button>
			</div>
		</div>
	</div>
	<div class="card">
		<div class="card__image">
			<img src="/src/assets/saco.jpg" alt="poncho">
		</div>
		<div class="card__info">
			<h3>Poncho super cool</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione error repellat explicabo deleniti modi fugit, veniam laboriosam, beatae minus inventore similique qui esse. Voluptatibus magnam fuga doloribus, praesentium natus possimus.</p>
			<div class="buy__container">
				<span class="price">$39.99</span>
				<button class="btn btn__bag">Add to the bag</button>
			</div>
		</div>
	</div>
	<div class="card">
		<div class="card__image">
			<img src="/src/assets/saco.jpg" alt="poncho">
		</div>
		<div class="card__info">
			<h3>Poncho super cool</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione error repellat explicabo deleniti modi fugit, veniam laboriosam, beatae minus inventore similique qui esse. Voluptatibus magnam fuga doloribus, praesentium natus possimus.</p>
			<div class="buy__container">
				<span class="price">$39.99</span>
				<button class="btn btn__bag">Add to the bag</button>
			</div>
		</div>
	</div>
	<div class="card">
		<div class="card__image">
			<img src="/src/assets/saco.jpg" alt="poncho">
		</div>
		<div class="card__info">
			<h3>Poncho super cool</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione error repellat explicabo deleniti modi fugit, veniam laboriosam, beatae minus inventore similique qui esse. Voluptatibus magnam fuga doloribus, praesentium natus possimus.</p>
			<div class="buy__container">
				<span class="price">$39.99</span>
				<button class="btn btn__bag">Add to the bag</button>
			</div>
		</div>
	</div>
	<div class="card">
		<div class="card__image">
			<img src="/src/assets/saco.jpg" alt="poncho">
		</div>
		<div class="card__info">
			<h3>Poncho super cool</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione error repellat explicabo deleniti modi fugit, veniam laboriosam, beatae minus inventore similique qui esse. Voluptatibus magnam fuga doloribus, praesentium natus possimus.</p>
			<div class="buy__container">
				<span class="price">$39.99</span>
				<button class="btn btn__bag">Add to the bag</button>
			</div>
		</div>
	</div>
	<div class="card">
		<div class="card__image">
			<img src="/src/assets/saco.jpg" alt="poncho">
		</div>
		<div class="card__info">
			<h3>Poncho super cool</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione error repellat explicabo deleniti modi fugit, veniam laboriosam, beatae minus inventore similique qui esse. Voluptatibus magnam fuga doloribus, praesentium natus possimus.</p>
			<div class="buy__container">
				<span class="price">$39.99</span>
				<button class="btn btn__bag">Add to the bag</button>
			</div>
		</div>
	</div>
</div>

<!-- Here ends xd -->



<?php include_once './src/Components/Footer.php';  ?>